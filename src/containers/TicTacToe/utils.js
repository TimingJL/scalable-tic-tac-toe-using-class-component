...
const getWinCase = (blocks, arrs, winnerCondition) => {
    ...
    arrs.forEach((arr) => {
        ...
        arr.forEach((element) => {
            ...
            if (isContinuous) {
                if (currentPlayer) {
                    ...
                    // prev undefinded, current undefined
                    // do nothing
                    winCaseArr = [];
                }
            } else {
                if (currentPlayer) {
                    // prev O, current X
                    // prev undefined, current X
                    // prev X, current O
                    // prev undefined, current O
                    ...
                    // prev O, current undefined
                    // prev undefined, current undefined
                    // prev X, current undefined
                    // prev undefined, current undefined
                    if (prevPlayer && (winCaseArr.length >= winnerCondition)) {
                        result[prevPlayer].push([...winCaseArr]);
                    }
        ...
        if (winCaseArr.length >= winnerCondition) { // wrap line case
